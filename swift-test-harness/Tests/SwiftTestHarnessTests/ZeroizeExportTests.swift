...
// of the pipeline:
//
//   1. `embedSwiftExportForXcode` produced `Zeroize.swiftmodule/`
...
//      `import Zeroize` above would fail at compile time.
//
//   2. The static archive `libZeroize.a` (produced by the
...
//      architecture arm64".
//
//   3. The Kotlin `swiftExport { moduleName = "Zeroize" }` and
//      `flattenPackage = "io.github.kotlinmania.zeroize"` configuration in
...
//      via the `ZeroizeLibrary` product.
//
...
final class ZeroizeExportTests: XCTestCase {
    func testSwiftModuleLoads() throws {
        XCTAssertTrue(true, "Zeroize swift module imported cleanly")
    }
}
