# Claude Code Project Instructions — zeroize-kotlin

## Project Overview

This is **zeroize-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`zeroize`](https://crates.io/crates/zeroize).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/zeroize/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/zeroize>
