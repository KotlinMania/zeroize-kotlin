...
Kotlin Multiplatform line-by-line clean-room port of the Rust crate [`zeroize`](https://crates.io/crates/zeroize).

> **Status: scaffold — porting has not started.** This repo currently contains build infrastructure only. The upstream Rust source for the `zeroize` crate will be cloned into `tmp/` (gitignored) when porting begins.

## About

Zero-on-drop sensitive data
...
```kotlin
dependencies {
    implementation("io.github.kotlinmania:zeroize-kotlin:0.1.0-SNAPSHOT")
}
```
...
Original work copyrighted by the upstream `zeroize` authors. Kotlin port copyright (c) 2026 Sydney Renee and The Solace Project.
